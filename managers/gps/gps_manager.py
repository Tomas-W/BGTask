...
class GPSManager:
    ...
    Supports distance-based monitoring for location alerts.
    """
    ...
    STATIONARY_INTERVAL = 6000    # 6 seconds in milliseconds
    MOVING_INTERVAL = 2000        # 2 seconds in milliseconds
    MIN_MOVEMENT_DISTANCE = 50     # 50 meters to consider "moving"
    DEFAULT_ALERT_DISTANCE = 1000  # 1000 meters alert radius
    ...
    def _wait_for_location(self, callback: Callable[[float, float], None]) -> None:
        ...
        self._location_event.wait(self.LOCATION_TIMEOUT)
    ...
    def _start_location_service(self, callback: Callable[[float, float], None]) -> None:
        ...
        try:
            ...
            def on_location(lat, lon):
                ...
                callback(lat, lon)  # Call the callback directly
                logger.trace(f"Location updated: {lat}, {lon}")
