...
class TaskPopupContainer(BoxLayout):
    ...
    def _update_bg(self, instance, value: tuple[float, float]) -> None:
...
class TaskPopupLabel(Label):
    """
    A TaskPopupLabel is a label for displaying Task info in a Popup.
    Similar to TaskInfoLabel in visuals but without functionality.
    """
    def __init__(self, **kwargs):
        super().__init__(
            text="",
    ...
    def _update_text_size(self, instance, value):
        """Updates text_size when width changes to enable text wrapping."""
        self.text_size = (self.width, None)

    def _update_height(self, instance, value):
        """Updates widget height based on texture size."""
